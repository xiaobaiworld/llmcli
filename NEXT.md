...
- [ ] 创建 `data/apps.schema.json`（基于 DEV.md 中的数据结构）

### Step 2：飞书数据文件
- [ ] 创建 `data/apps/feishu.yaml`
...
### Step 3：企业微信数据文件
- [ ] 创建 `data/apps/wecom.yaml`
  - 工具：wecom-cli
  - 域：消息、日历、文档、智能表格、会议、待办、通讯录
  - 同上标准

### Step 4：Obsidian 数据文件
- [ ] 创建 `data/apps/obsidian.yaml`
...
### Step 6：组件
- [ ] 创建 `src/components/AppCard.astro` — 应用卡片（首页用）
- [ ] 创建 `src/components/CoverageRow.astro` — 功能覆盖行（矩阵行）
- [ ] 创建 `src/components/ToolBadge.astro` — CLI/MCP 工具标签
...
### Step 9：构建验证
- [ ] 运行 `npm run build`，确认零错误，零 warning
- [ ] 检查生成页面数量（应 = 3 个 App 详情页 + 首页 + 50 个旧工具页 + ...）
